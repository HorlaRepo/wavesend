...
public class CountryServiceImpl implements CountryService {
    ...
    @Override
    public ApiResponse<Set<MobileMoneyOptions>> getMobileMoneyOptionsByCountryAcronym(String acronym) {
        Country country = countryRepository.findByAcronym(acronym).orElseThrow(()-> new ResourceNotFoundException("Country not supported"));
        return  ApiResponse.<Set<MobileMoneyOptions>>builder()
                .success(true)
                .message("Mobile money options found")
                .data(country.getMobileMoneyOptions())
                .build();
    }
}
